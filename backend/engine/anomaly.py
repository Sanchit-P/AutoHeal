...
# Train IsolationForest on raw baseline features
...
def detect_anomaly(cpu: float, memory: float, network: float):
    ...
    # IsolationForest signal (on raw features)
    ...
    # Z-score ensemble on raw features against baseline statistics
    mu = BASELINE.mean(axis=0)
    sigma = BASELINE.std(axis=0) + 1e-6
    z = np.abs((data[0] - mu) / sigma)
